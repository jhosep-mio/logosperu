...
export const metadata = {
  title: 'Portafolio de SEO - Logos Perú',
  description:
      'Nos especializamos en elevar tu presencia en línea a través de estrategias de SEO (Search Engine Optimization) eficaces y personalizadas. En este video, exploramos cómo nuestro servicio de posicionamiento web puede transformar tu negocio, aumentando la visibilidad y atrayendo a más clientes.',
  msvalidate: 'A01D8EBEAA551809606CCFFE234E6DF5',
  alternate: {
    canonical: 'https://logosperu.com.pe/portafolio/diseno-de-paginas-web/seo'
  },
  themeColor: '#4e54c8',
  ...
  openGraph: {
    title: 'Portafolio de SEO - Logos Perú',
    description:
        'Nos especalizamos en elevar tu presencia en línea a través de estrategias de SEO (Search Engine Optimization) eficaces y personalizadas. En este video, exploramos cómo nuestro servicio de posicionamiento web puede transformar tu negocio, aumentando la visibilidad y atrayendo a más clientes.',
    url: 'https://www.logosperu.com.pe/portafolio/diseno-de-paginas-web/seo',
...
const getData = async (texto: string) => {
  try {
    ...
    console.error('Error fetching data from getSubcategoriaUrlToPortafolio', error)
    throw error
  }
}
...
const getData3 = async (id: string) => {
  try {
    ...
    console.error('Error fetching data from getItemsToPortafolioWhereSubCategorias with ID', error)
    throw error
  }
}
      ...
      <section className='max-w-[1200px] mx-auto mb-24'>
        <div className='mb-16' id='userefprimercontenido'>
          <ul className='w-full flex flex-wrap items-center justify-center text-3xl py-5'>
            {subcategorias.filter((sub) => sub.id == 19).map((sub) => (
              <Link
                href={`/portafolio/diseno-de-paginas-web/${sub.url}`}
        ...
        <Contenido items={items} titulo={productos.titulo} ruta='diseno-de-paginas-web' />
      </section>
    </>
  )
}

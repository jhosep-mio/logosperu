...
export const ModalWhatsapp = ({
      ...
      try {
        ...
        setResumen((resumenesPrevios: valuesResumen[]): valuesResumen[] => {
          ...
          const enviarNotificacion = async (): Promise<void> => {
            ...
            data.append('contenido', `${auth.name} subio un nuevo comentario para el proyecto ${proyecto?.nombre_marca ?? ''}  (${proyecto?.nombres ?? ''} ${proyecto?.apellidos ?? ''})`)
  ...
  return (
    <>
      <section className="w-full flex flex-col justify-between fondo_wsp_s relative min-h-screen">
        ...
        <div className="h-fit w-full xl:pl-[14%]  left-0 right-0 flex items-end justify-center border border-t-gray-300 fixed bottom-0 z-20">
          ...
          ></textarea>
          {(auth.id == '8' || auth.id == '99' || auth.id_rol != 99) && !loading
            ? (
            <IoIosSend

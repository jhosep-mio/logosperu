...
export const ModalFechaAlta = ({
  ...
  const actualizar = async (): Promise<void> => {
    if (dateInicio) {
      ...
      data.append('fecha_alta', dateInicio ? formatDate(dateInicio) : null)
      data.append('_method', 'PUT')

      try {
        const respuesta = await axios.post(
          `${Global.url}/updateAlta/${id ?? ''}`,
          data,
          {
            headers: {
        ...
        if (respuesta.data.status == 'success') {
          Swal.fire(
            'Se agrego correctamente la fecha de alta',
            '',
            'success'
          )
          setOpen(false)
      ...
      Swal.fire('Debe seleccionar un fecha de alta', '', 'warning')
    }
  }

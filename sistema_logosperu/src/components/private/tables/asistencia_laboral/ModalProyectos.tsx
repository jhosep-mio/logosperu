...
export const ModalProyectos = ({ setOpen, setProyectoSeleccionado }: { setOpen: any, setProyectoSeleccionado: any }): JSX.Element => {
  const { auth } = useAuth()
  ...
  return (
    <section className='absolute z-[999] inset-0 bg-gray-200 py-2'>
      <div className="flex flex-row items-center justify-between gap-y-4 mb-3 md:mb-5 gap-2">
        ...
        <div className='p-3'>
            <FaRegWindowClose className='text-2xl text-red-500 cursor-pointer hover:text-main_dark transition-colors' onClick={() =>
              setOpen(false)}/>
        </div>
      </div>
      {loading
        ...
        <div className="md:bg-[#fff] md:p-8 rounded-xl">
          ...
          {filterDate().map((orden: ValuesVenta, index: number) => (
            <div
              ...
              onClick={() => setProyectoSeleccionado(orden)}
              key={orden.id}
            >
              <div

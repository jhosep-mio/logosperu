...
export const Login = (): JSX.Element | undefined => {
  ...
  if (auth.id !== '') {
    ...
    const validar = async (values: Values): Promise<void> => {
      ...
      try {
        const respuesta = await axios.post(`${Global.url}/loginClasificados`, data)

        if (respuesta.data.status === 'success') {
          setLoged('login')
          localStorage.setItem('tokenClasificado', respuesta.data.acces_token)
          localStorage.setItem('clienteClasificado', JSON.stringify({
    ...
    return (
      <div className="min-h-screen flex flex-col-reverse md:flex-row items-center justify-between">
        <div className="bg-[#1E1F25]  p-8 md:px-20  shadow-2xl w-screen lg:w-2/5 h-screen flex flex-col justify-center">
            ...
            <div className="flex flex-col w-full gap-2 relative ">
              <div className="w-full relative">
                <input
                  ...
                  onBlur={handleBlur}
                  className={`py-3 px-8 bg-[#131517] w-full outline-none rounded-lg text-gray-300  border  ${
                    errors.email !== null &&
        ...
        <div className="w-screen md:w-3/5  md:h-screen hidden lg:block">
          <img src={bg_video} alt="" className="w-full h-full object-cover"/>
        </div>
      </div>
    )
  }

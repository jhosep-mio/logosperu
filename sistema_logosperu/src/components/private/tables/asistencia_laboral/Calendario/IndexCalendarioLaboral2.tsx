...
export const IndexCalendarioLaboral2 = (): JSX.Element => {
  ...
  const components = {
    event: (props: any) => {
      return (
        ...
        >
          <div
            className={ cn('div_cita px-1 h-full text-white bg-[#54A9DC] transition-colors rounded-t-md') }
          >
            <span className="block lowercase first-letter:uppercase text-sm" onClick={() => { setActiveDescription(true) }}>
              {props.title}
            </span>
          </div>
        </div>
  ...
  return (
    <>
      ...
      {loading
        ...
        <Calendar
            className={`calendario_cm2 text-black calendario_horario_laboral ${activeDescription ? 'activeDescription' : ''} ${fullView ? 'fullCalendar' : ''}`}
            localizer={localizer}
            events={events}
        ...
        >
          ...
          <button type="button" className={`absolute top-[65px] left-[40px] text-xl mb-2 text-[#54A9DC] ${fullView ? 'hidden' : 'block'}`} onClick={() => { setFullView(true) }}><SlSizeFullscreen/></button>
          <button type="button" className={`absolute top-[20px] left-[10px] text-xl mb-2 text-[#54A9DC] ${fullView ? 'block' : 'hidden'}`} onClick={() => { setFullView(false) }}><SlSizeActual/></button>

      <ViewHorario2 open={open} setOpen={setOpen} activeDescription={activeDescription} fullScreen={fullView}/>
      </section>}
    </>
  )
}

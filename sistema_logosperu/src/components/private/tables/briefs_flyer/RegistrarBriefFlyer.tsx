...
export const RegistrarBriefFlyer = (): JSX.Element => {
  ...
  const saveBrief = async (values: valuesFlyer): Promise<void> => {
    ...
    } else if (!option3Checked && !option4Checked && !option5Checked && !option6Checked) {
      Swal.fire('', 'Debe seleccionar el ripo de flyer', 'warning')
    } else if (
      (option2Checked && values.medida1.length == 0) ||
  ...
  useEffect(() => {
    setTitle('REGISTRAR BRIEF DE FLYER')
  }, [])

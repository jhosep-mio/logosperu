...
export const GeneracionCorrelativo = ({
  ...
  const generarCodigo = async (plan: string): Promise<void> => {
    ...
    try {
      ...
      if (request.data.status == 'success') {
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPPG')) {
          descripcionEntrada =
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPCM')) {
          descripcionEntrada =
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPBRO')) {
          descripcionEntrada =
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPTV')) {
          descripcionEntrada =
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPREEL')) {
          descripcionEntrada =
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPFLYER')) {
          descripcionEntrada =
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPSEO')) {
          descripcionEntrada =
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPADS')) {
          descripcionEntrada =
        ...
        } else if (formatearContrato(request.data.codigo).includes('LPWA')) {
          descripcionEntrada =
        ...
        console.log(formatearContrato(request.data.codigo))
        setValues({
          ...values,
  ...
  return (
    <>
      ...
      >
        <section
          className={`grid ${pdfUrl ? 'grid-cols-2' : 'grid-cols-1'}  gap-10`}
        >
          {values.id_contrato ? (
            <ModalContratos
              datos={values}

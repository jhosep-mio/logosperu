...
import { SchemaClasificado } from '../../../shared/Schemas'
import {
  type ImagenState,
...
import { ImageUpdateClasificados } from '../../../shared/ImageUpdateClasificados'
import { AgregarProductos } from '../../../shared/AgregarProductos'
import { AgregarMarca } from '../../../shared/AgregarMarca'
...
export const ViewResultados = (): JSX.Element => {
  ...
  const getClasificado = async (): Promise<void> => {
    const token = localStorage.getItem('tokenClasificado')

    const request = await axios.get(`${Global.url}/oneWeb/${auth.id ?? ''}`, {
      headers: {
    ...
    const arrayproducto = JSON.parse(request.data.pagina_web).productos
      ?.productos
      ? JSON.parse(JSON.parse(request.data.pagina_web).productos?.productos)
      : []
    const arraymarcasTwo = JSON.parse(request.data?.pagina_web).marcas
      ? JSON.parse(JSON.parse(request.data?.pagina_web).marcas)
      : []
    setValues({
      ...values,
  ...
  const savePreventa = async (
    ...
    try {
      const respuesta = await axios.post(
        `${Global.url}/updateClasificado/${auth.id ?? ''}`,
        data,
        {
          headers: {
            Authorization: `Bearer ${
                ...
                <div className="mb-3 md:mb-0 w-full rounded-md rounded-tl-none  text-black flex flex-col items-end ">
                  ...
                  >
                    <AccordionSummary
                      ...
                      id="panel2bh-header"
                    >
                      <h2 className="font-bold text-base uppercase">BANNERS</h2>
                  ...
                  >
                    <AccordionSummary
                      ...
                      id="panel7bh-header"
                    >
                      <h2 className="font-bold text-base uppercase">MARCAS</h2>
                    </AccordionSummary>
                    <AccordionDetails>
                      <div className="w-full flex gap-3 justify-between items-center px-4 py-2">
                        <button

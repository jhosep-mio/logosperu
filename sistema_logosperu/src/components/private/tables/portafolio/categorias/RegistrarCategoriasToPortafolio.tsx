...
import { SchemaCategoriasToPortafolio } from '../../../../shared/schemas/Schemas'
...
export const RegistrarCategoriasToPortafolio = (): JSX.Element => {
  const { id } = useParams()
  ...
  const saveCategoria = async (): Promise<void> => {
    if (arrayPesos.length == 0) {
      Swal.fire('No se puede registrar una categorias sin imagenes/videos', '', 'warning')
    } else {
      setLoading(true)
      ...
      try {
        ...
        `${Global.url}/registrarCategoriaToPortafolio`,
        data,
        {
          headers: {
        ...
        if (respuesta.data.status == 'success') {
          ...
          navigate('/admin/categorias-portafolio')
        } else {
  ...
  const {
    ...
    validationSchema: SchemaCategoriasToPortafolio,
    onSubmit: saveCategoria
  })

  const getOneBrief = async (): Promise<void> => {
    const request = await axios.get(
      `${Global.url}/oneCategoriaToPortafolio/${id ?? ''}`,
      {
        headers: {
          Authorization: `Bearer ${
    ...
    setarrayPesos(request.data.imagen1 ? JSON.parse(request.data.imagen1) : [])
    setLoading(false)
  }
      ...
      {loading
        ...
        <form className="bg-white p-8 rounded-xl mt-5" onSubmit={handleSubmit}>
          ...
          <div className="flex gap-2 w-full justify-end">
            ...
            <Link
              to="/admin/categorias-portafolio"
